...
resource "aws_route53_record" "route53" {
  ...
  name    = "${var.dns_host_entry}.${var.domain_name}"
  type    = "CNAME"
  ttl     = "60"
  records = [join("", aws_elasticache_replication_group.default.*.primary_endpoint_address)]
  #records = ["${element(concat(list(replace(replace(jsonencode(aws_elasticache_cluster.redis.cache_nodes), "/.*address...([a-z0-9\\.\\-]+)\".*/" , "$1"), "[]", "")), list("")), 0)}"]
}
